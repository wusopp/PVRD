...
bool NvDecoder::init(AVFormatContext *pFormatCtx,int bUseInterop, int bTCC) {
    ...
    bool bLoad = loadVideoSource(pFormatCtx);
    if (!bLoad) {
        printf("数据源加载失败！\n");
        system("pause");
        return false;
...
bool NvDecoder::loadVideoSource(AVFormatContext *pFormatCtx) {
    ...
    if (!apVideoSource->init(pFormatCtx, apFrameQueue.get())) {
        return false;
    };
...
bool NvDecoder::copyDecodedFrameToTexture(OutputBuffer *output) {
    ...
    if (g_pFrameQueue->dequeue(&oDisplayInfo)) {
        ...
        for (int active_field = 0; active_field < num_fields; active_field++) {
           ...
           // CUresult result = cuMemcpyDtoHAsync(output->data, pDecodedFrame[active_field], (nDecodedPitch * nHeight * 3 / 2), g_ReadbackSID);
            if (result != CUDA_SUCCESS) {
                printf("cuMemAllocHost returned %d\n", (int)result);
                checkCudaErrors(result);
            }
...
HRESULT NvDecoder::reinitCudaResources() {
    ...
    bool bLoad = loadVideoSource(NULL);
    if (!bLoad) {
        printf("数据源加载失败！\n");
        system("pause");
        return S_FALSE;
...
void NvDecoder::SaveFrameAsYUV(unsigned char *pdst,
  ...
  //  }
    ...
    //if (cnt > 148) {
    //    FILE *fp = fopen ("small.yuv", "ab");
    //    fwrite (pdst, 1, width*height + (width*height) / 2, fp);
    //    fflush (fp);
    //    fclose (fp);
    //}
}

#include "NvDecoder.h"
#define HARDDECIDE2RENDER 1
#if HARDDECIDE2RENDER
#include "cuda.h"   
...
extern cudaGraphicsResource_t cudaResources;
cudaArray_t cuArray;
...
bool NvDecoder::copyDecodedFrameToTexture(unsigned char** ppImgData, int height, int width, GLuint &cudaTextureID, HDC &mainDC, HGLRC &mainGLRC, bool &needCudaMalloc) {
    ...
    if (g_pFrameQueue->dequeue(&oDisplayInfo)) {
        ...
        unsigned char* pImgData = *ppImgData;
        for (int active_field = 0; active_field < num_fields; active_field++) {
            ...
            */
            ...
            // 处理完即可接触资源锁定. OpenGL利用得的Texture进行纹理贴图操作
            ...
            if (make) {
                std::cout << "DecoderThread make NULL ok" << std::endl;
            } else {
                std::cout << "DecoderThread make NULL error" << std::endl;
            }

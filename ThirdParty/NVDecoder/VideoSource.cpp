...
bool VideoSource::init(AVFormatContext *pFormatCtx, FrameQueue *pFrameQueue,int w,int h)
{
    ...
    pCodecCtx->width = w;
    pCodecCtx->height = h;
    pCodecCtx->coded_width = w;
    pCodecCtx->coded_height = h;

    //pCodecCtx->time_base.den = 25;//fps
    //pCodecCtx->max_b_frames = 1;


    g_stFormat.coded_width = pCodecCtx->coded_width;//需要额外的编码参数集传进来
    g_stFormat.coded_height = pCodecCtx->coded_height;//需要额外的编码参数集传进来

    g_stFormat.display_area.right = pCodecCtx->width;//需要额外的编码参数集传进来
    g_stFormat.display_area.left = 0;
    g_stFormat.display_area.bottom = pCodecCtx->height;//需要额外的编码参数集传进来
